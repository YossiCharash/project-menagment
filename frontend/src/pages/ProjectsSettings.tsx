...
import { CategoryAPI, Category, SupplierAPI, Supplier, SupplierCreate } from '../lib/apiClient'
...
export default function ProjectsSettings() {
  ...
  const canSeeCategories = isAdmin || permissions.some(p => p.resource_type === 'category')
  const canSeeSuppliers = isAdmin || permissions.some(p => p.resource_type === 'supplier')
  ...
  // Reset forms and load data when switching tabs
  useEffect(() => {
    ...
    if (activeTab === 'categories' && canSeeCategories) {
      fetchCategories()
    } else if (activeTab === 'suppliers' && canSeeSuppliers) {
      fetchSuppliers()
  ...
  useEffect(() => {
    if (activeTab === 'categories' && !canSeeCategories && canSeeSuppliers) {
      setActiveTab('suppliers')
    } else if (activeTab === 'suppliers' && !canSeeSuppliers && canSeeCategories) {
      setActiveTab('categories')
    }
  }, [canSeeCategories, canSeeSuppliers, activeTab])
  ...
  if (!canSeeCategories && !canSeeSuppliers) return null
  ...
  return (
    <SettingsOverlay
      title="הגדרות פרויקטים"
      subtitle="ניהול קטגוריות וספקים"
      maxWidth="max-w-3xl"
      auxiliaryContent={deleteModals}
    >
      {/* Tabs */}
      <div className="border-b border-gray-200 dark:border-gray-700 -mx-5 px-5 -mt-5 pt-4 mb-5">
        <div className="flex flex-wrap gap-2 sm:gap-4">
          {canSeeCategories && (
            <button
              onClick={() => setActiveTab('categories')}
              className={`px-4 py-2 font-medium transition-colors border-b-2 flex-shrink-0 whitespace-nowrap -mb-px ${
